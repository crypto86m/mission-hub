 ...
 * 5 sections: System Health, Lead Pipeline, Compliance Audit, API Status, Alpaca Positions
 */
...
async function fetchAlpacaAccount(mode = 'paper') {
  const configs = {
    ...
    live: {
      ...
      key: 'AKGRWTAEPYPFMKJMZFMFTVQNO6',
      secret: '35fPtVPABQfALi9re6dsrr2YF1V6v2nyw3CDa5DsY54L',
    }
  };
...
function StatusBadge({ status, label }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${
      isOn ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-red-500/20 text-red-400 border border-red-500/30'
    }`}>
    ...
    <div className="p-4 pb-24">
      ...
      {section === 'health' && (
        <>
          ...
          <div className={`glass-card p-3 mb-3 border-l-4 ${
            !health?.alerts?.length ? 'border-l-green-500' : 'border-l-yellow-500'
          }`}>
          ...
          {health?.alerts?.map((a, i) => (
            <div key={i} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs mb-2 ${
              a.level === 'critical' ? 'bg-red-500/10 border border-red-500/30 text-red-300' : 'bg-yellow-500/10 border border-yellow-500/30 text-yellow-300'
            }`}>
          ...
          <div className="grid grid-cols-2 gap-2.5 mb-3">
            <MetricCard icon={Wifi} title="Gateway" value={health?.gateway?.status === 'online' ? '✅ Live' : '❌ Down'}
              subtitle={`${health?.gateway?.memory_mb || 0}MB / 4GB`}
              color={health?.gateway?.status === 'online' ? 'green' : 'red'} />
      ...
      {/* ===== SECTION: ALPACA POSITIONS ===== */}
      {section === 'positions' && (
        <>
          {/* Account Summary */}
          <div className="grid grid-cols-2 gap-2.5 mb-3">
            <MetricCard icon={BarChart2} title="Paper Account" value={`$${paperVal.toLocaleString('en-US', {minimumFractionDigits:2})}`}
              subtitle={paperAccount ? `Buying Power: $${parseFloat(paperAccount.buying_power).toLocaleString()}` : 'Loading...'}
              color="cyan" />
            <MetricCard icon={BarChart2} title="Live Account" value={`$${liveVal.toLocaleString('en-US', {minimumFractionDigits:2})}`}
              subtitle={liveAccount ? `Buying Power: $${parseFloat(liveAccount.buying_power).toLocaleString()}` : 'Loading...'}
              color="green" />
          </div>
          ...
          <div className={`glass-card p-3 mb-3 border-l-4 ${totalUnrealized >= 0 ? 'border-l-green-500' : 'border-l-red-500'}`}>
            <div className="flex items-center justify-between">
              ...
              <span className={`text-lg font-bold ${totalUnrealized >= 0 ? 'text-green-400' : 'text-red-400'}`}>
          ...
          <SectionCard title={`Open Positions (${positions.length})`} icon={TrendingUp}>
            {positions.length === 0 && !posError && (
              <p className="text-xs text-gray-500 py-2">No open positions</p>
            )}
            {posError && (
              <p className="text-xs text-red-400 py-2">⚠️ {posError}</p>
            )}
            {positions.map((pos, i) => {
          ...
          <SectionCard title="Account Details" icon={BarChart2}>
            ...
            <Row label="PDT Protection" status={paperAccount?.pattern_day_trader === false ? true : false} label2="Safe" />
          </SectionCard>
        </>
      )}
      ...
      {section === 'leads' && (
        <>
            ...
            {[
              ...
              <div key={i} className="flex items-center justify-between py-2.5 border-b border-gray-800/50 last:border-0">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <StatusBadge status={p.leads > 0} label={p.leads > 0 ? 'Active' : 'Empty'} />
                </div>
              </div>
            ))}
          </SectionCard>
          ...
          <SectionCard title="Pipeline Metrics" icon={BarChart2}>
            <Row label="Emails Sent" value="0" />
            <Row label="Reply Rate" value="—" />
            <Row label="Meetings Booked" value="0" />
      ...
      {section === 'audit' && (
        <>
          <div className="grid grid-cols-2 gap-2.5 mb-3">
            ...
            <MetricCard icon={Lock} title="Integrity" value={health?.compliance?.integrity === 'verified' ? '✅ Verified' : '⚠️ Check'}
              subtitle="SHA-256 hash validation" color="green" />
          </div>
          ...
          <SectionCard title="Tracked Actions" icon={FileText}>
            {[
              ...
              { name: 'API Calls', icon: '🔌', count: 0 },
              { name: 'Lead Outreach', icon: '📧', count: 0 },
          ...
          <SectionCard title="Recent Entries" icon={Clock}>
            <div className="text-[10px] text-gray-500 space-y-1.5">
              ...
              <p>[12:05] config_changed: Lead pipeline created</p>
              <p>[12:05] credential_updated: Alpaca Live API key stored</p>
              <p>[12:05] credential_updated: Alpha Vantage API key stored</p>
              <p>[12:05] credential_updated: ElevenLabs API key stored</p>
              <p>[12:05] credential_updated: Tavily API key stored</p>
        ...
        <>
          ...
          <SectionCard title="MCP Server Status" icon={Brain}>
            ...
            <div className="mt-2 text-[10px] text-gray-500">
              All MCP servers configured with auto-restart (3s delay, max 5/min)
            </div>
          </SectionCard>
          ...
          <SectionCard title="Credential Security" icon={Shield}>
            ...
            <Row label="Total Credentials" value={`${Object.values(health?.api_status || {}).filter(Boolean).length + 9} saved`} />
