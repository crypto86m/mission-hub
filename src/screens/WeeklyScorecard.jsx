...
import { Trophy, TrendingUp, TrendingDown, DollarSign, AlertTriangle, CheckCircle, Clock, Zap, RefreshCw } from 'lucide-react';
...
export default function WeeklyScorecard() {
  ...
  // Build scorecard from live data
  ...
  const systemScores = [
    ...
    { name: "Bennett's Brief", score: nl.subscribers > 0 ? 85 : null, trend: 'stable', detail: `${nl.subscribers} subscribers, ${nl.issuesPublished} issues` },
  ...
  return (
    <div className="w-full h-full overflow-y-auto pb-24 px-4 pt-6">
      ...
      <div className="mb-6">
        ...
        {!liveData && <p className="text-xs text-red-400 mt-1">⚠️ Could not load live data</p>}
      </div>
      ...
      <div className="mb-6">
        <div className="glass-card">
          ...
          {tradingConnected ? (
            ...
            <p className="text-gray-500 text-sm">Awaiting connection to trading platform</p>
          )}
        </div>
      </div>
      ...
      {(ig.status === 'not_connected' || tw.status === 'not_connected' || co.rlm?.status === 'not_connected') && (
        <div className="mb-6">
          ...
          <div className="space-y-1.5">
            ...
            {co.rlm?.status === 'not_connected' && (
              <div className="glass-card flex items-start gap-2 py-2 border-yellow-500/10">
                <span className="text-yellow-400 text-xs mt-0.5">⚠</span>
                <span className="text-xs text-gray-300">RLM Enterprises — No data source connected. Metrics unavailable.</span>
              </div>
            )}
            {co.nvcc?.status === 'not_connected' && (
              <div className="glass-card flex items-start gap-2 py-2 border-yellow-500/10">
                <span className="text-yellow-400 text-xs mt-0.5">⚠</span>
                <span className="text-xs text-gray-300">NVCC — No data source connected. Metrics unavailable.</span>
              </div>
            )}
          </div>
        </div>
